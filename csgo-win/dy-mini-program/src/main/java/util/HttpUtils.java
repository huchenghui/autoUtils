...
public class HttpUtils {
    ...
    public static String runExcelCase(String testCase,String token) throws Exception {
        String resp;
        ...
        else if ("post".equalsIgnoreCase(method))
        {
            if (Strings.isNullOrEmpty(transferType)) {
                resp = analysis(doPost(url, reqData, token));
            }
            else {
        ...
        {
            if (Strings.isNullOrEmpty(transferType)) {
                resp = analysis(doPut(url, reqData, token));
            }
            else {
        ...
        {
            resp = analysis(doDelete(url,reqData,token));
        }
        else
        {
    ...
    public static Object[][] dataProvider(String filePath,int sheetNo,String funcName) throws Exception {
        ...
        String parameterData = list.get(0).get("parameter_data").toString();
        int size = 0;
        if (parameterData.startsWith("{"))
        {
            size = JSON.parseObject(parameterData).size();
        }
        else if (parameterData.startsWith("["))
        {
            size = JSON.parseArray(parameterData).size();
        }
        else { throw new Exception("startsWith is not { or ["); }
        Object[][] objs = new Object[list.size()][size+1];
        LinkedHashMap<String,Object> json;
        String paramData;
        List<String> list1;
        JSONObject dataDriver;

        for (int i = 0; i < list.size(); i++)
        {
            dataDriver = new JSONObject(list.get(i));
            LOG.info("from {} ddt is {}",filePath,dataDriver);
            paramData = list.get(i).get("parameter_data").toString();
            LOG.info("parameter_data is {}",paramData);
            if (paramData.startsWith("{"))
            {
                json = JSON.parseObject(paramData, new TypeReference<LinkedHashMap<String, Object>>() {
                });
                ...
                {
                    objs[i][j] = json.get(keys.get(j));
                }
                objs[i][json.size()] = dataDriver.getString("expected_result");
            }
            else if (paramData.startsWith("["))
            {
                list1 = JSON.parseObject(paramData,new TypeReference<LinkedList<String>>(){});
                for (int j = 0;j < list1.size();j++)
                {
                    objs[i][j] = list1.get(j);
                }
                objs[i][list1.size()] = dataDriver.getString("expected_result");
            }
            else { throw new Exception("row "+i+"startsWith is not { or ["); }
            keys.clear();
        }
        return objs;
    }
